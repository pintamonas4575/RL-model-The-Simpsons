...
class RL_Agent_42():
    ...
    def __init__(self, game_env: Scratch_Game_Environment4):
        ...
        self.target_dqn = Custom_DQN(self.num_states, self.num_actions).to(device)
        self.policy_dqn = Custom_DQN(self.num_states, self.num_actions).to(device)
    ...
    def replay(self) -> None:
        ...
        # NOTE: el problema es que a veces una celda puede ser terminal y otras no
...
agent = RL_Agent_42(game_env=my_env)
...
path_to_save = f"V4_version/V4_2_DQN_{agent.game_env.total_squares}_{EPISODES}"
step_counter = 0
...
for i in range(EPISODES):
    ...
    while not done:
        ...
        neighbors, reward, done = agent.game_env.env_step(cell_index=indice_celda_actual)
        move_action_index = agent.choose_action(indice_celda_actual)
        indice_proxima_celda = neighbors[move_action_index]
        agent.remember(indice_celda_actual, move_action_index, reward, indice_celda_actual, done)
        agent.replay()
    ...
    if i % trace == 0 or i == EPISODES-1:
        ...
        agent.game_env.get_window_image_and_save(True, f"episodes/V4_2_episode_{i}.png")
...
torch.save(agent.policy_dqn.state_dict(), f"results/V4_version/policy_{agent.game_env.total_squares}_{EPISODES}.pth")
torch.save(agent.target_dqn.state_dict(), f"results/V4_version/target_{agent.game_env.total_squares}_{EPISODES}.pth")
